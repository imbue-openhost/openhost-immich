...
as a Cloud in a Bottle app, with an in-container OIDC bridge that gates
sign-in on the Cloud in a Bottle compute-space owner session.
...
- **OIDC bridge** -- a small Starlette + PyJWT service that
  ...
  authenticated when Cloud in a Bottle has stamped them with
...
1. User visits `https://immich.<zone>/`. Cloud in a Bottle's perimeter does
...
3. Immich's web client redirects the user to
   `/_oidc/authorize?...`. Our OIDC bridge sees the Cloud in a Bottle owner
...
5. The user is in. Subsequent visits skip everything: the Cloud in a Bottle
   session cookie keeps the request authenticated; Immich's own
   session cookie keeps the user logged in until either expires.

If the Cloud in a Bottle session is missing (the user is not signed in), the
bridge bounces them to `https://<zone>/login`, Cloud in a Bottle handles the
...
- **Single-tenant.** Cloud in a Bottle itself is single-owner today, so the
  ...
  instance is not supported until Cloud in a Bottle grows multi-user
  identity.
- **No mobile app SSO.** The Immich mobile apps' OAuth callback
  scheme (`app.immich:///oauth-callback`) needs the user to be
  signed into Cloud in a Bottle in their mobile browser at the time, which
...
- `openhost.toml` -- Cloud in a Bottle manifest (port 8080, app_data, 6 GiB
  RAM, 3 cores).
...
- The OIDC bridge listens on `127.0.0.1:9000` so external traffic
  cannot bypass nginx and skip the Cloud in a Bottle owner check.
...
- The `passwordLogin.enabled: false` Immich setting means a leaked
  ...
  through our bridge, which requires a Cloud in a Bottle owner session.
